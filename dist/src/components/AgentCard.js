...
export function AgentCard({ session, width }) {
    ...
    const duration = session.sessionStartedAt
        ...
        : '';
    return (_jsxs(Box, { flexDirection: "column", width: width, borderStyle: "round", borderColor: color, paddingX: 1, children: [_jsxs(Text, { bold: true, wrap: "truncate", children: [session.repoName || session.projectName, session.gitBranch ? (_jsxs(Text, { dimColor: true, children: [" (", session.gitBranch, ")"] })) : null] }), _jsx(Text, { dimColor: true, italic: true, wrap: "truncate-end", children: session.taskSummary || ' ' }), _jsx(Box, { justifyContent: "center", marginY: 1, children: _jsx(Text, { color: color, children: character.art }) }), _jsx(Box, { height: 2, overflow: "hidden", marginBottom: 1, children: _jsx(Text, { dimColor: true, wrap: "wrap", children: session.lastResponseText || ' ' }) }), _jsx(Text, { color: color, wrap: "truncate", children: session.statusText }), session.currentFile ? (_jsxs(Text, { dimColor: true, wrap: "truncate", children: ["File: ", session.currentFile] })) : null, session.activeSubagents > 0 ? (_jsxs(Text, { color: "magenta", children: ["Subagents: ", session.activeSubagents] })) : null, session.contextTokens > 0 ? (_jsxs(Text, { dimColor: true, children: ["Context: ", formatTokens(session.contextTokens), ' ', "(", Math.round((session.contextTokens / session.contextMaxTokens) * 100), "%)"] })) : null, duration ? (_jsxs(Text, { dimColor: true, children: ["Session: ", duration] })) : null, session.toolHistory.length > 0 ? (_jsxs(Box, { flexDirection: "column", marginTop: 1, children: [_jsx(Text, { dimColor: true, children: "Recent:" }), session.toolHistory.map((entry, i) => (_jsxs(Text, { dimColor: true, wrap: "truncate", children: [i === session.toolHistory.length - 1 ? ' > ' : '   ', entry.status] }, i)))] })) : null] }));
}
